...
function get_function_verify($post_data) {
    // clean array
    ...
    $conn = conn();
    $check = "fail";
    if ($result = mysqli_query($conn, "SELECT * FROM verify WHERE key='" . $clean_data["appheader"]["key"] . "' AND code='" . $clean_data["appheader"]["datablock"]["verify"]["verifycode"] . "'  LIMIT 1")) {
        while ($row = mysqli_fetch_array($result)) {
            $check = $row["verifycode"];
        }
...
switch ($view) {
    ...
    break;
    case "verify":
        $function_output = get_function_verify($post_data);
    break;
    case "":
        //404 - not found;//
        $function_output["status"] = 404;
