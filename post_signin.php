...
if ($clean_data["appheader"]["datablock"]["signin"]["passwordhash"]==true){$check="Success";}
}
else
//facebook//	
...
$signin_method="Native";
	...
	if ($result = mysqli_query($conn, "SELECT * FROM setting WHERE emailhash='".$clean_data["appheader"]["datablock"]["signin"]["emailhash"]."' AND passwordhash='".$passwordhash."'  LIMIT 1")) 
	{
		while($row = mysqli_fetch_array($result))
		 {  
			if(sodium_crypto_pwhash_str_verify($row["passwordhash"], $clean_data["appheader"]["datablock"]["signin"]["passwordhash"])) {$check="Success";}
		 } 	
