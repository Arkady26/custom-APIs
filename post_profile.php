...
function get_function_profile($post_data) {
    // clean array
    ...
    $conn = conn();
    $field_array = $clean_data["appheader"]["datablock"]["profile"];
    $where = "WHERE key='" . $clean_data["appheader"]["key"] . "'";
    $sql_opd = build_sql_update("setting", $field_array, $where);
...
switch ($view) {
    ...
    break;
    case "profile":
        $function_output = get_function_profile($post_data);
    break;
    case "":
        //404 - not found;//
        $function_output["status"] = 404;
